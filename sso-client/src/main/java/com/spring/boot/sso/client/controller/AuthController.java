...
public class AuthController {
    ...
    public void check(String token, String returnUrl, HttpSession session, HttpServletResponse response) throws IOException {
        ...
        System.out.println("请求了: [ client ] /auth/check");
        System.out.println("token: " + token + ", returnUrl: " + returnUrl);
        String sessionId = session.getId();
        RestTemplate rest = new RestTemplate();
        String url = ssoHost + "/auth/verify?token=" + token + "&localId=" + sessionId;
        String result = rest.getForObject(url, String.class);
        String host = UrlUtil.getClientHost(returnUrl);
        if ("token无效".equals(result)) {
            response.sendRedirect(ssoHost + "/page/login?redirect=http://" + host + "/auth/check&returnUrl=" + returnUrl);
            return;
        }
    ...
    public void logout(String localId, HttpSession session, HttpServletResponse response) throws IOException {
        System.out.println("请求了: [ client ] /auth/logout");
        String sessionId = session.getId();
        RestTemplate rest = new RestTemplate();
        String url = ssoHost + "/auth/logout?localId=" + sessionId;
